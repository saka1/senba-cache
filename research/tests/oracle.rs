...
fn c18s_1shard_matches_orig_on_synthetic_zipf() {
    ...
    for &(skew, cap) in &[(1.05_f64, 16usize), (1.1, 32), (1.2, 64), (1.5, 64)] {
        let trace: Vec<u64> = ZipfGen::new(skew, 10_000, 42).take(200_000).collect();
        let mut a: sieve_orig::SieveCache<u64, u64> = sieve_orig::SieveCache::new(cap);
        let b: C18s<u64, u64, 1> = C18s::new(cap);
        for k in &trace {
            a.insert(*k, *k);
            b.insert(*k, *k);
        }
        ...
        eprintln!(
            "[c18s vs orig] skew={skew} cap={cap}: diff={diff}/{} ({:.4}%)",
            trace.len(),
            100.0 * diff as f64 / trace.len() as f64
        );
        total_diff += diff;
    ...
    assert_eq!(
        total_diff, 0,
        "c18s が sieve_orig と divergent ({total_ops} ops 中 {total_diff} diff): \
         versions 別配列化 (G2-α-2) が SIEVE 不変条件を破っている (= 設計通りでない)"
    );
}
