//! `sieve_c13s`: c12s の不採用判定を踏まえ、senba::Cache (ST library) の structural
//! skeleton に c11s/c12s で確立した並行 layer を載せ、さらに **Path A (existing key
//! update) のみ lock-free CAS 化** した変種。
//!
//! # 動機 (c11s / c12s からの位置取り)
//!
//! c11s は reader を lock-free 化したが writer は Mutex で直列化、read-heavy zipf 16T
//! で 18 Mops に plateau (`docs/reports/2026-05-08-c11s-conditional-set.md` §5)。
//! c12s は writer 全体を lock-free CAS 化したが、install-at-evicted-pos が SIEVE 順序
//! を破壊し HR -10〜-72%、不採用となった (`docs/reports/2026-05-08-c12s-cas-slot-claim.md`)。
//!
...
//! 保持される。
//!
//! c13s は **Path A だけ lock-free**、**Path B/C は writer Mutex 配下で senba::Cache
//! 流 shift-on-evict** で実装。SIEVE 等価性を保ったまま hot key write contention を
//! 緩和する。
//!
...
//!
//! CAS 失敗 = 並行 writer (別の Path A or Path B/C) が tag を変更 → MAX_RETRY (=4) まで
//! find からやり直し、超えたら writer Mutex に escalate。
//!
...
//!
//! c13s は research artifact (= bench で V = u64 を使う) としては sound、API 表面は
//! V: Clone を許容する形にしてある。
//!
//! # miri
//!
...
const fn id_shift_from_entry_size(s: usize) -> u32 {
    assert!(
        s.is_power_of_two(),
        "sieve_c13s: sizeof(Entry<K,V>) must be a power of two"
    );
    assert!(s <= 256, "sieve_c13s: sizeof(Entry<K,V>) must be <= 256");
    s.trailing_zeros()
}
    ...
    V: Clone,
{
    ...
    /// AVX2 path は使わず scalar (Path A は perf path だが、SIMD 経由でも seqlock 再検証が
    /// 必要なので寄与は小さい; c12s も find_lockfree は scalar)。
...
mod tests {
    ...
    /// senba::Cache の shift-on-evict と同じ ID 再利用を c13s でも守っているかの確認。
    ...
    /// c13s は SIEVE 等価なのでここで一致する。詳細は research/tests/oracle.rs。
    #[test]
    fn matches_sieve_orig_externally_1shard() {
        ...
        for k in 0..256u64 {
            assert_eq!(
                a.get(&k).copied(),
                b.get(&k),
                "1-shard で sieve_orig と c13s が key {k} で食い違う"
            );
        }
    }
