...
//! `sieve_r1`: r-series 1st variant — shard 間 routing に thread affinity を導入した
//! c17s 派生。設計仕様: `docs/reports/2026-05-12-r1-design.md`。
//!
//! # c17s からの差分
//!
//! c17s の単シャード concurrency 機構 (`Shard` / `ShardHot` / `Slot16` / AVX2 scan /
//! Path A/B/C / 2-tier seqlock / Path C epoch) は **100% そのまま継承** する。差分は
//! 外側の routing 関数のみ:
//!
...
//! `WAYS=1` は c17s と完全に等価 (= sanity check 専用 path)、`WAYS=4` で T=4 まで
//! shard 内 hot-line bouncing 構造除去、`WAYS=8` で T=8 まで、というスケーリング。
//!
...
//! # HR drop の trade-off
//!
//! Hard scheme (本実装) は (i) bouncing-free と (iii) probe O(1) を取り、(ii) HR 等価を
//! 犠牲にする。同一 key を thread t1, t2 が触ると別 shard に行き duplicate 化する。
//! 1 thread 視点の effective cap = total_cap / WAYS。per-thread keyspace ワークロードでは
//! HR drop ≈ 0、shared hot key では最大 WAYS 倍 duplicate 化。設計 §6.2 の (T × WAYS)
//! sweep で workload 別 HR drop を可視化することが主成果物。
//!
//! # 内部詳細 (c17s から不変)
//!
//! tag layout、Entry::version seqlock、Path A/B/C state machine、SIMD scan、AVX2 必須、
//! `V: !Copy` (`String` 等) soundness の議論は c17s のドキュメンテーション
//! (`sieve_c17s.rs`) および `docs/reports/2026-05-11-c17s-design.md` を参照。
...
impl<K, V, const SHARDS: usize> ConcurrentSieveR1<K, V, SHARDS>
where
    K: Hash + Eq,
    V: Clone,
{
    ...
    /// 2 引数 routing。`hash` の低位 bit で set、`tls` の低位 bit で way を選ぶ。
    /// `ways=1` のとき `way_mask=0` で `way=0`、`set_mask = SHARDS-1` となり c17s と一致。
    #[inline]
    fn shard_of(&self, hash: u64, tls: u32) -> usize {
        let set = (hash as usize) & self.set_mask;
        let way = (tls as usize) & self.way_mask;
        set * self.ways + way
    }
...
mod tests {
    ...
    impl crate::experimental::ConcurrentCacheImpl<u64, u64> for ConcurrentSieveR1<u64, u64> {
    ...
    fn shard_of_at_ways1_matches_c17s_hash_only_routing() {
        let cache: ConcurrentSieveR1<u64, u64, 64> = ConcurrentSieveR1::with_ways(64, 1);
    ...
    /// `ways=4` で同一 hash でも tls の低位 2 bit ごとに 4 つの隣接 shard に
    /// 分散することを確認 (r1 のハード分割の core invariant)。
    #[test]
    fn shard_of_at_ways4_disperses_by_tls() {
        let cache: ConcurrentSieveR1<u64, u64, 64> = ConcurrentSieveR1::with_ways(64, 4);
        let h: u64 = 0xDEADBEEF;
        let set = (h as usize) & 0x0F; // sets = 16, set_mask = 15
        let base = set * 4;
        let mut seen = std::collections::HashSet::new();
        for tls in 0u32..4 {
            let s = cache.shard_of(h, tls);
            assert!(
                (base..base + 4).contains(&s),
                "tls={tls} shard={s} outside set base..base+4 = [{base},{base}+4)"
            );
            seen.insert(s);
        }
        assert_eq!(
            seen.len(),
            4,
            "ways=4 should route 4 tls values to 4 shards"
        );
    }

    /// `ways=SHARDS` (= 全 way、1 set) 構成: set_mask=0, way_mask=SHARDS-1。
    /// hash に依らず tls だけで shard が決まる極端構成のサニティ。
    #[test]
    fn shard_of_at_ways_equals_shards_ignores_hash() {
        let cache: ConcurrentSieveR1<u64, u64, 64> = ConcurrentSieveR1::with_ways(64, 64);
        for tls in 0u32..64 {
            let s_a = cache.shard_of(0x0, tls);
            let s_b = cache.shard_of(u64::MAX, tls);
            assert_eq!(
                s_a, s_b,
                "tls={tls} should map to same shard regardless of hash"
            );
            assert_eq!(s_a, tls as usize);
        }
    }

    #[test]
    fn evicts_oldest_when_full_and_unvisited() {
        let cache: ConcurrentSieveR1<i32, i32, 1> = ConcurrentSieveR1::new(2);
        cache.insert(1, 10);
        cache.insert(2, 20);
    ...
    fn warm_up_to_steady_transition() {
        let cache: ConcurrentSieveR1<u64, u64, 1> = ConcurrentSieveR1::new(4);
    ...
    fn distinct_keys_full_per_shard_all_hit() {
        let n: u64 = 64;
        let cache: ConcurrentSieveR1<u64, u64, 1> = ConcurrentSieveR1::new(n as usize);
        for k in 0..n {
            cache.insert(k, k * 7);
        }
        for k in 0..n {
    ...
    fn concurrent_invariants_under_zipf() {
        ...
        let cap = 256usize;
        let cache: Arc<ConcurrentSieveR1<u64, u64, 8>> = Arc::new(ConcurrentSieveR1::new(cap));
    ...
    fn matches_sieve_orig_externally_1shard() {
        ...
        let b: ConcurrentSieveR1<u64, u64, 1> = ConcurrentSieveR1::new(cap);
