//! `sieve_c15s`: c14s に **sloppy visited gate** を追加した派生 (Phase 1 of
//! `docs/reports/2026-05-10-write-contention-design-space.md` §7)。
//!
//! reader hit 時の `visited.fetch_or` を `1 / 2^SAMPLE_BITS` の確率でのみ発火
//! させ、hot key 上での cache line ping-pong を構造的に減らす。SAMPLE_BITS=0
//! では gate が compile-time に dead-eliminate され、c14s と同 codegen を狙う
//! (= baseline)。
//!
//! 変更箇所:
//! - reader hot path (`try_candidate` 内 1 箇所): 確率 gate で wrap
//! - writer Path A 成功時の visited SET / `scan_evict` のクリア: **無変更**
//!   (orig `freq=1` 等価性と SIEVE state machine 維持のため)
//!
//! 設計の一次資料: `docs/reports/2026-05-10-write-contention-design-space.md`。
//! c14s 由来の 3 点 tuning (find_lockfree AVX2 / MAX_RETRY=1 / reader bounded
//! retry) はそのまま継承。
//!
//! # 動機 (c13s からの位置取り)
//!
...
const fn id_shift_from_entry_size(s: usize) -> u32 {
    assert!(
        s.is_power_of_two(),
        "sieve_c15s: sizeof(Entry<K,V>) must be a power of two"
    );
    assert!(s <= 256, "sieve_c15s: sizeof(Entry<K,V>) must be <= 256");
    s.trailing_zeros()
}
...
pub struct Shard<K, V, const SAMPLE_BITS: u32 = 0> {
    ...
    tags: Box<[AtomicU16]>,
    /// VISITED bit を pos 単位で bit-packed した独立配列 (c11s 由来)。
    /// reader hit 時の `fetch_or` はこちらに発火し、tags 列を MESI Shared 維持できる。
    ...
    /// senba::Cache lineage で tail field なし。
    len: AtomicUsize,
    /// Path B (warmup install) と Path C (evict + shift + install) の排他状態。
    /// Path A (lock-free CAS update) は Mutex を取らない。
...
unsafe impl<K: Send, V: Send, const B: u32> Send for Shard<K, V, B> {}
unsafe impl<K: Send + Sync, V: Send + Sync, const B: u32> Sync for Shard<K, V, B> {}

impl<K, V, const SAMPLE_BITS: u32> Shard<K, V, SAMPLE_BITS> {
    ...
    V: Clone,
{
    ...
    fn try_candidate(&self, pos: usize, key: &K, needle: u16) -> Probe<V> {
        ...
        if buf.key == *key {
            ...
            // c15s sloppy gate: SAMPLE_BITS=0 で gate は compile-time dead-eliminate、
            // SAMPLE_BITS>0 で 1/(2^SAMPLE_BITS) 確率でのみ visited fetch_or を撃つ。
            // hot key の MESI ping-pong を構造的に削減する (write traffic を 2^N 倍希釈)。
            // 内側の load-then-fetch_or は c11s 由来 (already-set ならば fetch_or skip)。
            if SAMPLE_BITS == 0 || (next_rand() & ((1u64 << SAMPLE_BITS) - 1)) == 0 {
    ...
    fn writer_evict_and_install(
        ...
        debug_assert_eq!(self.len.load(Ordering::Relaxed), cap);
        // hand を範囲内に修正
        if state.hand >= cap {
            state.hand = 0;
        }
        ...
        for i in evict_pos..(cap - 1) {
            ...
            let (sw, sb) = Self::vbit(i + 1);
            let was_visited = self.visited[sw].load(Ordering::Relaxed) & sb != 0;
            // 旧位置の visited を CLEAR
            self.visited[sw].fetch_and(!sb, Ordering::Relaxed);
            // 新位置の visited を was_visited で SET / CLEAR
            let (dw, db) = Self::vbit(i);
            if was_visited {
                self.visited[dw].fetch_or(db, Ordering::Relaxed);
            } else {
                self.visited[dw].fetch_and(!db, Ordering::Relaxed);
            }
...
impl<K, V, const B: u32> Drop for Shard<K, V, B> {
    fn drop(&mut self) {
        let len = self.len.load(Ordering::Relaxed);
        let entries_mut = self.entries.get();
        for i in 0..len {
...
impl<K, V, const SHARDS: usize, const SAMPLE_BITS: u32>
    ConcurrentSieveCache<K, V, SHARDS, SAMPLE_BITS>
where
    K: Hash + Eq,
    V: Clone,
{
    pub fn new(capacity: usize) -> Self {
        ...
        let shards: [Shard<K, V, SAMPLE_BITS>; SHARDS] = std::array::from_fn(|i| {
    ...
    #[cfg(test)]
    pub(crate) fn shard(&self, idx: usize) -> &Shard<K, V, SAMPLE_BITS> {
        &self.shards[idx]
    }
}

// ---------------- TLS wyrand (sloppy gate 用 RNG) ----------------

/// Per-thread wyrand state。最初の `next_rand()` 呼び出し時に
/// `SEED_CTR` から fetch_add で初期化される (thread 跨ぎでの seed 衝突回避)。
///
/// 1 call ≈ 5 instructions (TLS load + 2 mul + 2 xor)、AtomicU64 RMW を 1 度
/// だけ初回に踏むので steady state では cross-thread sync なし。
static SEED_CTR: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0x9E37_79B9_7F4A_7C15);

thread_local! {
    static R: std::cell::Cell<u64> = const { std::cell::Cell::new(0) };
}
...
fn next_rand() -> u64 {
    R.with(|c| {
        let mut s = c.get();
        if s == 0 {
            // 初回: 単調 counter から per-thread seed を切り出す。
            // | 1 で 0 (= uninit sentinel) との衝突を避ける。
            s = SEED_CTR.fetch_add(0x9E37_79B9_7F4A_7C15, std::sync::atomic::Ordering::Relaxed) | 1;
        }
...
/// テスト / bench で TLS RNG を deterministic に reseed する。
/// `seed | 1` を thread-local cell に直接書き込む (counter は使わない)。
pub fn reseed_for_test(seed: u64) {
    R.with(|c| c.set(seed | 1));
}

// ---------------- 公開 type alias (bench / test 用) ----------------

/// SAMPLE_BITS=0 (= c14s 等価、gate は dead-eliminate される)。
pub type ConcurrentSieveCacheC15S0<K, V, const SHARDS: usize> =
    ConcurrentSieveCache<K, V, SHARDS, 0>;
/// SAMPLE_BITS=2 (= 1/4 sample)。
pub type ConcurrentSieveCacheC15S4<K, V, const SHARDS: usize> =
    ConcurrentSieveCache<K, V, SHARDS, 2>;
/// SAMPLE_BITS=3 (= 1/8 sample)。
pub type ConcurrentSieveCacheC15S8<K, V, const SHARDS: usize> =
    ConcurrentSieveCache<K, V, SHARDS, 3>;
/// SAMPLE_BITS=4 (= 1/16 sample)。
pub type ConcurrentSieveCacheC15S16<K, V, const SHARDS: usize> =
    ConcurrentSieveCache<K, V, SHARDS, 4>;
