...
//! `sieve_r2h`: r1 の hash-routed control — thread affinity を捨て、way を
//! `mix(routing_hint, hash) mod ways` で選ぶ。設計仕様: `docs/reports/2026-05-13-r2-design.md` §3.1。
//!
//! # r1 からの差分
//!
//! r1 の `shard_of(hash, tls) = set(hash) * ways + (tls & way_mask)` を、
//! r2h では `shard_of(hash, tls) = set(hash) * ways + (mix(tls, hash) & way_mask)`
//! に置き換える。`mix` は `tls.wrapping_mul(0x9E37_79B9_7F4A_7C15) ^ hash` の
//! multiplicative-XOR で、tls を hash と独立な軸として混ぜる。
//!
//! # 役割: コントロール実験
//!
//! r2h は採用候補ではなく **r1 の affinity 寄与を切り分ける control**。同一 key を
//! 異 thread が触ると別 way に行く点は r1 と同じだが、同 thread の連続 insert は
//! key に応じて全 way に uniformly に散る (r1 では同 way に集約)。これにより:
//!
//! - r1 の主利得 (per-way writer state が thread 固定 → L1 ピン留め) が消失するか確認
//! - 残るのは "ways 倍 sub-cache" の効果のみ → `senba::Cache(shards = SHARDS × ways)`
//!   と等価仮説 (H2) を直接観測
//!
//! 詳細仮説と判定基準: `docs/reports/2026-05-13-r2-design.md` §4。
//!
//! # 内部詳細 (r1 / c17s から不変)
//!
//! Shard 内の concurrency 機構 (`ShardHot` / `Slot16` / AVX2 scan / Path A/B/C /
//! 2-tier seqlock / Path C epoch) は **100% そのまま継承**。`with_ways(capacity, 1)`
//! で c17s と bit-for-bit 等価動作 (way_mask=0 で mix の結果が常に way=0 に縮退)。
...
pub struct ConcurrentSieveR2h<K, V, const SHARDS: usize = DEFAULT_SHARDS> {
...
impl<K, V, const SHARDS: usize> ConcurrentSieveR2h<K, V, SHARDS>
where
    K: Hash + Eq,
    V: Clone,
{
    /// 主 constructor。`ways` は power-of-2、`1 <= ways <= SHARDS` を満たすこと。
    pub fn with_ways(capacity: usize, ways: usize) -> Self {
        ...
        assert!(
            ...
            "ways ({ways}) must be a power of two"
        );
        assert!(ways <= SHARDS, "ways ({ways}) must be <= SHARDS ({SHARDS})");
        assert!(
            capacity >= SHARDS,
            "capacity ({capacity}) must be >= SHARDS ({SHARDS}) so each shard has cap >= 1"
        );
        let sets = SHARDS / ways;
        let set_mask = sets - 1;
        let way_mask = ways - 1;
    ...
    /// `WAYS=1` 既定の thin shim — c17s と routing が等価。
    pub fn new(capacity: usize) -> Self {
        Self::with_ways(capacity, 1)
    }
    ...
    /// r2h routing。`hash` の低位 bit で set、way は `mix(tls, hash)` の低位 bit。
    /// `ways=1` のとき `way_mask=0` で `way=0` に縮退、c17s と等価。
    ///
    /// `mix` は `tls.wrapping_mul(golden_ratio) ^ hash`。golden ratio 定数
    /// `0x9E37_79B9_7F4A_7C15` で tls の低位 bit を全 64 bit に拡散させ、`^ hash` で
    /// key 軸を混ぜる。これにより同 thread の連続 insert が key に応じて全 way に
    /// uniform に散る (r1 では同 thread が同 way に集約されるのと対比)。
    #[inline]
    fn shard_of(&self, hash: u64, tls: u32) -> usize {
        let set = (hash as usize) & self.set_mask;
        let mixed = (tls as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15) ^ hash;
        let way = (mixed as usize) & self.way_mask;
        set * self.ways + way
    }
...
mod tests {
    ...
    impl crate::experimental::ConcurrentCacheImpl<u64, u64> for ConcurrentSieveR2h<u64, u64> {
    ...
    // NOTE: `concurrent_suite!` registration is intentionally omitted per the r1
    // design (`docs/reports/2026-05-12-r1-design.md` §4.7): r1 acceptance is gated
    // on bench-driven HR/Mops pareto, not on the c-series concurrent invariant suite.
    // The single `concurrent_invariants_under_zipf` test below is retained as a
    // smoke check for shard-level uniqueness / LIVE consistency.

    /// `ways=1` で routing が c17s と bit-for-bit 等価動作することを 1000 random hash
    /// で確認 (Risk #1 — mask 計算 off-by-one を `with_ways` ガード前段で検出)。
    #[test]
    fn shard_of_at_ways1_matches_c17s_hash_only_routing() {
        let cache: ConcurrentSieveR2h<u64, u64, 64> = ConcurrentSieveR2h::with_ways(64, 1);
        for seed in 0u64..1000 {
            let h = seed.wrapping_mul(0x9E3779B97F4A7C15);
            let expected = (h as usize) & 63; // c17s: `shard_of_hash`
            // tls の値に関わらず way_mask=0 で way=0 になる。
            for tls in [0u32, 1, 7, 42, u32::MAX] {
                let got = cache.shard_of(h, tls);
                assert_eq!(got, expected, "h={h:#x} tls={tls}");
            }
        }
    }

    /// `ways=4` で同一 tls + 異 hash が複数 way に散ることを確認 (r2h の core invariant:
    /// hash 軸が way 選択に効く)。r1 では同一 tls なら常に同 way だったのが r2h では
    /// hash で散る点が差別化のポイント。
    #[test]
    fn shard_of_at_ways4_disperses_by_hash_for_fixed_tls() {
        let cache: ConcurrentSieveR2h<u64, u64, 64> = ConcurrentSieveR2h::with_ways(64, 4);
        let tls: u32 = 7;
        let mut way_hits: std::collections::HashMap<usize, usize> =
            std::collections::HashMap::new();
        for seed in 0u64..256 {
            let h = seed.wrapping_mul(0x9E3779B97F4A7C15);
            let s = cache.shard_of(h, tls);
            let set = (h as usize) & 0x0F;
            let base = set * 4;
            assert!((base..base + 4).contains(&s), "shard {s} out of set");
            let way = s - base;
            *way_hits.entry(way).or_insert(0) += 1;
        }
        // 4 ways に概ね均等 (各 ~64 hits ±20%) 分散するはず。
        assert_eq!(way_hits.len(), 4, "all 4 ways should be hit by hash sweep");
        for (&w, &n) in &way_hits {
            assert!(
                (40..=100).contains(&n),
                "way {w} hit {n} times, expected ~64"
            );
        }
    }

    /// `ways=64` (= 全 way、1 set) 構成: set_mask=0, way_mask=SHARDS-1。
    /// r2h では同一 tls でも hash で way が決まるので、hash が変われば shard も変わる。
    #[test]
    fn shard_of_at_ways_equals_shards_disperses_by_hash() {
        let cache: ConcurrentSieveR2h<u64, u64, 64> = ConcurrentSieveR2h::with_ways(64, 64);
        let tls: u32 = 3;
        let s_a = cache.shard_of(0x0, tls);
        let s_b = cache.shard_of(u64::MAX, tls);
        assert_ne!(
            s_a, s_b,
            "r2h: different hash should yield different shard even for fixed tls"
        );
    }

    #[test]
    fn evicts_oldest_when_full_and_unvisited() {
        let cache: ConcurrentSieveR2h<i32, i32, 1> = ConcurrentSieveR2h::new(2);
        cache.insert(1, 10);
        cache.insert(2, 20);
    ...
    fn warm_up_to_steady_transition() {
        let cache: ConcurrentSieveR2h<u64, u64, 1> = ConcurrentSieveR2h::new(4);
    ...
    fn distinct_keys_full_per_shard_all_hit() {
        let n: u64 = 64;
        let cache: ConcurrentSieveR2h<u64, u64, 1> = ConcurrentSieveR2h::new(n as usize);
        for k in 0..n {
            cache.insert(k, k * 7);
        }
        for k in 0..n {
    ...
    fn concurrent_invariants_under_zipf() {
        ...
        let cap = 256usize;
        let cache: Arc<ConcurrentSieveR2h<u64, u64, 8>> = Arc::new(ConcurrentSieveR2h::new(cap));
    ...
    fn matches_sieve_orig_externally_1shard() {
        ...
        let b: ConcurrentSieveR2h<u64, u64, 1> = ConcurrentSieveR2h::new(cap);
