//! `sieve_c11s`: c10s から派生した **conditional visited set** variant。
//!
//! # c10s との差分 (本 variant の核心)
//!
//! c10s は reader hit ごとに `visited[w].fetch_or(b, Relaxed)` を撃つ。x86 の
//! `lock or` は **値が変わるかどうかに関係なく** cache line を MESI Modified に
//! 遷移させ、他コアが Shared で保持していれば invalidate を必ずブロードキャスト
//! する。zipf hot key のように visited=1 が定常状態の slot でも、全 reader が
//! 同 word の cache line を取り合う ping-pong が発生していた。これが
//! `docs/reports/2026-05-08-c10s-vs-c8-baseline.md` で read-heavy zipf 16T が
//! c10s で -17 〜 -21% regress した主要因。
//!
//! c11s は reader hit を **load → 0 ならだけ fetch_or** の conditional 形式に
//! 変える:
//!
//! ```ignore
...
//! ```
//!
//! - 既に 1 (zipf hot key の定常状態) なら **load のみ** = MESI Shared で読める
//!   → 他コアの cache line を invalidate しない → ping-pong が消える
//! - 0 なら従来通り `fetch_or`。初回 hit と writer reset 直後の最初の hit のみ
//!
//! load + branch のコストは数 cycle、`lock or` の coherency traffic と比較すれば
//! 桁違いに小さい。SIEVE eviction 順序は完全保持 (oracle 一致)。c10s と直交軸の
//! 単一行 diff に近いので、効きの有無を独立検証する。
//!
//! 旧 c10s docstring (継承する性質): VISITED bit を tag 列の外に分離し、
//! tags 列を reader からは Acquire load 専用に保つ構造は c11s でもそのまま継承。
//!
//! # Tag bit layout
//!
//! c8 と c11s で 1 bit ずれる:
//!
//! - c8:    `LIVE (1) | VISITED (1) | id (6) | hash (8)` = 16 bit
//! - c11s:  `LIVE (1) | id (6) | hash (9)` = 16 bit (旧 VISITED bit は hash に転用)
//!
...
//!   c11s では reader が tag を変更しないので phantom 概念自体が消える。コード簡素化。
...
const fn id_shift_from_entry_size(s: usize) -> u32 {
    assert!(
        s.is_power_of_two(),
        "sieve_c11s: sizeof(Entry<K,V>) must be a power of two"
    );
    assert!(s <= 256, "sieve_c11s: sizeof(Entry<K,V>) must be <= 256");
    s.trailing_zeros()
}
    ...
    V: Copy,
{
    ...
    fn try_candidate(&self, pos: usize, key: &K, needle: u16) -> Option<V> {
        ...
        if entry.key == *key {
            ...
            // conditional set: 既に立っていれば書き込まない (= cache line を MESI
            // Shared のまま残し、他コアの invalidate を回避)。c10s との唯一の差分。
    ...
    fn writer_update_in_place(&self, pos: usize, key: K, value: V) {
        ...
        //    c11s では LIVE | id | hash の 3 区画のみで VISITED bit が無いので
    ...
    fn matches_sieve_orig_externally_1shard() {
        ...
        for k in 0..256u64 {
            assert_eq!(
                a.get(&k).copied(),
                b.get(&k),
                "1-shard で sieve_orig と c11s が key {k} で食い違う"
            );
        }
    }
    ...
    fn matches_j8_externally_1shard() {
        ...
        for k in 0..256u64 {
            assert_eq!(
                a.get(&k).copied(),
                b.get(&k),
                "1-shard で j8 と c11s が key {k} で食い違う"
            );
        }
    }
