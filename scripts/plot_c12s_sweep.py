"""Plot c12s (CAS-based slot claim) single-shard sweep.

Inputs:  docs/reports/data/2026-05-08-c12s-sweep.csv
Outputs: docs/reports/data/2026-05-08-c12s-{metric}-{op_mix}.png
         docs/reports/data/2026-05-08-c12s-summary.csv
"""
...
DATA = ROOT / "docs" / "reports" / "data" / "2026-05-08-c12s-sweep.csv"
OUTDIR = ROOT / "docs" / "reports" / "data"

VARIANTS = ["c8", "c11s", "c12s"]
COLORS = {"c8": "#1f77b4", "c11s": "#9467bd", "c12s": "#d62728"}
MARKERS = {"c8": "o", "c11s": "D", "c12s": "s"}
...
def main() -> None:
    ...
    grp.to_csv(OUTDIR / "2026-05-08-c12s-summary.csv", index=False)
    ...
    for op_mix in op_mixes:
        for metric, ylabel, fname_tag in [
            ...
            fig.tight_layout()
            out = OUTDIR / f"2026-05-08-c12s-{fname_tag}-{op_mix}.png"
    ...
    # 採否判定: read-heavy zipf-1.0 16T で c12s と c8 の比較
    print("\n=== 採否判定: read-heavy zipf-1.0 16T (c8 vs c12s) ===")
    judge = grp[
    ...
    if not judge.empty:
        print(judge[["variant", "aggregate_mops", "mops_min_per_thread", "p99_chunk_ns"]].to_string(index=False))
        c8 = judge[judge["variant"] == "c8"]["aggregate_mops"]
        c12s = judge[judge["variant"] == "c12s"]["aggregate_mops"]
        if not c8.empty and not c12s.empty:
            ratio = float(c12s.iloc[0]) / float(c8.iloc[0])
            print(f"\nc12s / c8 = {ratio:.3f} ({(ratio - 1.0) * 100:+.1f}%)")
            if ratio >= 1.05:
                print("→ 採用 (c8 を 5%+ 上回る)")
            elif ratio >= 0.95:
                print("→ 並 (±5%)")
