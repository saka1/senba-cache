...
pub struct ConcurrentSieveCache<K, V> {
    ...
    shard_mask: usize,                    // shards.len() - 1、shard_of_hash は単一 AND
...
- **SHARDS は runtime** (= senba::Cache (ST) と同形の `Box<[...]>` + `shard_mask`)。
  c8 のように const generic にはしない。理由: 容量 → 最適 shard 数のスイートスポット
  (per_shard ∈ {16, 32, 64} あたりが SIEVE の経験則上の幅) は cap によって変化する
  ため、bench で動かすパラメータとして runtime 可変にする必要がある
...
shard 数を bench から指定する `with_shards`。
...
where
    ...
    V: Send + Sync,
{
    pub fn new(capacity: usize) -> Self;             // auto-shard (senba::Cache 流)
    pub fn with_shards(capacity: usize, shards: usize) -> Self;  // bench 用 override

    pub fn capacity(&self) -> usize;
    pub fn len(&self) -> usize;                      // 各 shard の Mutex を 1 回ずつ取って合計
    pub fn is_empty(&self) -> bool;
    pub fn shards(&self) -> usize;                   // = shard_mask + 1
...
`new` は senba::Cache の auto-shard ロジック (`n_min = ceil(capacity / MAX_PER_SHARD)`、
`shards = n_min.next_power_of_two()`) をそのまま継承する。bench で per-shard を
スイープしたい場合は `with_shards` を直接呼ぶ。
...
c9 は runtime shard なので、c8 のような **const-generic dispatch (`match args.shards`) は不要** —
`with_shards(capacity, args.shards)` で 1 行で構築できる。

ただし既存 trait の `build(capacity: usize) -> Arc<Self>` のままでは shard 数を
渡せないため、**`ConcCache::build` を `build(capacity: usize, shards: usize) -> Arc<Self>`
に拡張**する。c8 側 (const generic) は既存の dispatch でそのまま、moka / mini-moka 側は
`shards` 引数を無視する形で実装:
