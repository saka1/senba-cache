...
- **2026-05-06 追記**: §8 に D' (per_shard sweep) の結果を追加。§4.5 の予測「per_shard を下げると
  (b) false-match 退行が消えて (a) dep chain 単独になる」を実測で確認。**per_shard=16 では
  Δ(j8−j7) は平均 +0.1 ns まで縮み、cap=16384 では j8 が j7 を 1.55 ns 上回る**。
  raw: `profiles/j8_pershard_sweep_2026-05-06.csv`、script: `scripts/sweep_j8_pershard.sh`。
...
| # | アクション | 期待される情報 | 工数 | 状態 |
|---|---|---|---|---|
| **A** | **memfair sweep**: j8 cap=N vs orig cap=⌈1.25 N⌉ を per_shard=64 固定で 5 trial | 「memory-fair で j8 が orig を抜くか」の決着 | 30 分 | 未 |
| **D'** | **per_shard sweep**: per_shard ∈ {16, 32, 64} で j7 vs j8 を比較。§4.5 の予測「per_shard ↓ で false-match 退行が消えて (a) 単独になる」を実測で確認 | (a) と (b) の分解の確証 + j8 sweet spot 確定 | 30 分 | **完 (§8)** |
| E | cluster019 (scan-heavy / 低 hit) cell の比較 | hit ratio gain (+6.32 pp at cap=1024 in j5/j7) が ns/op としてどう寄与するか | 1 時間 | 未 |
...
D' は完了 (→ §8)。次に学びが大きいのは **A (memfair)**。j8 設計の主目的「memory-fair で
orig を absolute に抜く」への直接の答えで、D' と独立に走らせて良い。

## 8. D' per_shard sweep 結果 (2026-05-06)

### 8.1 セットアップ

- raw: `profiles/j8_pershard_sweep_2026-05-06.csv`
- script: `scripts/sweep_j8_pershard.sh`
- trace: cluster018 1M 行、SEED=42、TRIALS=5、median 採用 (5 trial の中央値)
- 軸: cap ∈ {1024, 4096, 16384} × per_shard ∈ {16, 32, 64} の 9 cell × {j7, j8}
- orig は per_shard 概念なしで cap=cap で 1 cell/cap

### 8.2 主表 (median ns/op)

| cap   | per_shard | shards | orig  | j7    | j8    | Δ(j8−j7) | Δ(j8−orig) |
|------:|---------:|------:|------:|------:|------:|---------:|-----------:|
| 1024  | 16       | 64    | 38.43 | 29.21 | 30.56 | **+1.34** | −7.88 |
| 1024  | 32       | 32    | 38.43 | 30.15 | 32.42 | +2.28    | −6.01 |
| 1024  | 64       | 16    | 38.43 | 31.71 | 36.47 | +4.76    | −1.96 |
| 4096  | 16       | 256   | 31.62 | 29.87 | 30.49 | **+0.62** | −1.13 |
| 4096  | 32       | 128   | 31.62 | 30.83 | 32.32 | +1.49    | +0.70 |
| 4096  | 64       | 64    | 31.62 | 30.96 | 39.35 | +8.39    | +7.73 |
| 16384 | 16       | 1024  | 30.23 | 29.70 | 28.15 | **−1.55** | **−2.08** |
| 16384 | 32       | 512   | 30.23 | 30.67 | 32.07 | +1.40    | +1.84 |
| 16384 | 64       | 256   | 30.23 | 32.21 | 32.83 | +0.61    | +2.60 |

per_shard 別の Δ(j8−j7) 平均:

| per_shard | Δ平均 | scan あたり期待 false-match Δ (j8 − j7) | (a)+(b) 机上予測 |
|---:|---:|---:|---:|
| 16 | **+0.14** | 16/256 − 16/16384 = +0.062 → ~+0.1 ns | (a) +1.0 + (b) +0.1 ≈ +1.1 |
| 32 | **+1.72** | 32/256 − 32/16384 = +0.123 → ~+0.2 ns | (a) +1.0 + (b) +0.2 ≈ +1.2 |
| 64 | **+4.59** | 64/256 − 64/16384 = +0.246 → ~+0.9 ns | (a) +1.2 + (b) +0.9 ≈ +2.1 |

### 8.3 解釈

**(b) false-match 成分は per_shard で消せることが確認できた**。per_shard=16 では Δ 平均
+0.14 ns で j7 と実質同等、cap=16384 帯では **j8 が j7 を 1.55 ns 上回る**。これは初稿 §4.5
の予測「per_shard を下げれば false-match 期待値が比例で減って (a) 単独になる」を実測で
裏付ける。

**(a) dep chain 成分は予測 +1.0 ns より小さい (per_shard=16 で実測 +0.14 ns、per_shard=32 で
+1.72 ns)**。これは (a) の見積もりが pessimistic だった可能性、または scan 短縮 (= 16-lane scan
は loop body を 1 周回しただけで終わる) に伴う loop overhead 削減が dep chain 延長と相殺
している可能性を示唆する。per_shard=32 で +1.7 ns 出ているのが「純粋な (a) + 小さな (b)」
の真値に近いと思われる。

per_shard=64 / cap=4096 で Δ +8.39 ns は初稿 §3 の +3.80 ns を上回る大きい外れ値。trial 5
本のうち 1 本が 44.3 ms (vs 中央値 39.3 ms) と上振れしているが median は他 4 本に近く、
trial 4 (41.6 ms) も平均を引き上げている。WSL 上の再現性 noise (CPU 周波数管理、page cache、
backgroud activity) の範囲とみなす。本論を覆す観察ではない (per_shard=64 は元々 j8 の worst
帯域として診断済)。

**vs orig の比較**: per_shard=16 では **3 cap 全てで j8 が orig を absolute に上回る**
(−7.88, −1.13, −2.08 ns)。これは memfair (= cap で揃えない比較) ではなく **同 cap の絶対
比較** での話だが、j8 の同 cap での弱点 (cap=4096/16384 での orig 負け) は per_shard 選択
だけで解消できることを意味する。

### 8.4 sweet spot の更新

初稿 §6 では「per_shard ≤ 64 が j8 の構造的上限」とだけ述べていたが、D' を経て:

- **j8 の throughput 上の sweet spot は per_shard=16** (= 6-bit ID 制限の下限近傍ではなく
  scan 短縮が効く下端)
- per_shard=32 は (a) 単独の +1〜+2 ns 退行帯
- per_shard=64 は (a)+(b) で +4〜+8 ns 退行帯、cap が大きいほど j7 と diff が縮む傾向

これは設計時に念頭に置いていた「per_shard=32〜64 を主たる sweet spot」(設計書 §1, §2)
とは逆向きの結論。**bench の主軸は per_shard=16 に置くのが正しい**。

### 8.5 含意

- **j8 を捨てる必要はない**。per_shard=16 を主軸にすれば throughput 退行はほぼ消え、
  inline 20 B/cap (orig 25、j7 36 比) の memory 優位がそのまま残る
- メモリ的には per_shard=16 でも変わらない (tag 数も entries 数も per_shard には依存しない、
  shards 数の Vec ヘッダだけ増えるが O(shards × 24 B) で全体に対して微小)
- 残る検証は **A (memfair)**。per_shard=16 を採用した上で j8 cap=N vs orig cap=1.25N の
  head-to-head を取れば、設計書 §8.6 の主張「memfair で j 系列が orig を absolute に抜く」
  に決着がつく
