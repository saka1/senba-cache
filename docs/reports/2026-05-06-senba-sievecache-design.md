# 2026-05-06 — `senba::Cache` ライブラリ化設計
...
本設計は **publishable な Rust crate API** としての ST 版 `senba::Cache` を
確定する。並行版 (= c8 系の後継) と `Cache` trait 再設計は本スペックの scope 外、
別スペックで「新 `Cache` の上に再構築」として独立に扱う。

## 2. TL;DR

- 新公開型: `senba::Cache<K, V, S: SlotSize = Slot32, const SHARDS: usize = 8>`
...
// src/lib.rs (追加部分)
pub mod sieve_cache;
pub use sieve_cache::{Cache, SlotSize, Slot16, Slot32, Slot64};
```

既存 `sieve_orig`, `sieve_v0`〜`v3`, `sieve_j3`〜`j8`, `sieve_c8` は手付かず。
利用者は `senba_cache::Cache` を import する (crate を `senba` に rename した場合は
`senba::Cache`、本スペック scope 外)。

#### 名前衝突解消: 既存 `Cache` trait の rename

現状 `src/cache.rs` に research/oracle 共通基盤として `pub trait Cache<K, V>` が
定義され `lib.rs` から re-export、12 の `sieve_*` modules がこれを impl している
(bench harness が variant 横断で uniform に回せるようにするため)。

新規 struct `senba_cache::Cache` を導入するには名前衝突するので、本スペックでは
**trait `Cache` を `CacheImpl` に rename**する:

```rust
// src/cache.rs
pub trait CacheImpl<K, V> { ... }   // 旧 trait Cache

// src/lib.rs
pub use cache::CacheImpl;            // (旧 pub use cache::Cache)
pub use sieve_cache::Cache;          // 新規 struct
```

同時に既存 12 sieve_* modules の `impl crate::Cache<K, V> for ...` を全部
`impl crate::CacheImpl<K, V> for ...` に書き換える (置換 12 行)。意味論変更なし、
シンボル名のみ。

`CacheImpl` は「cache implementation interface」を素直に表す名前で、トップの新
struct `Cache` (= 利用者向け SIEVE 実装) との役割分担が明示される。
...
```rust
pub struct Cache<K, V, S: SlotSize = Slot32, const SHARDS: usize = 8> {
...
// 99% の利用者: SLOT を意識しない
let mut c: Cache<u64, String> = Cache::new(1024);
...
let mut c: Cache<String, String, Slot64> = Cache::new(1024);
...
`sieve_orig` (1 shard) と新 `Cache::<K, V, S, 1>` で:
...
小さい K, V で memory の絶対競争したい利用者は `Cache::<K, V, Slot16>` を explicit に
...
- variant: `orig` / `sieve_j8` (= research baseline) / `Cache<u64, u64, Slot32>`
- string ワークロード: `(String, String)` で `Cache<_, _, Slot64>` を `moka` / `mini-moka` と AB
...
| 段階 | 内容 | ゲート |
|---|---|---|
| **0a** | 既存 trait `Cache` → `CacheImpl` rename (`src/cache.rs` + `lib.rs` re-export + 12 sieve_* impl 行) | `cargo test` 全 green (既存 test に影響なし) |
| 0b | `sealed` mod + `SlotSize` trait + `Slot16/32/64` ZST + `Slot*Storage` union 定義 | `cargo check` 通過 |
...
| 3 | `Cache` 公開型 + 既存 j8 テスト移植 + `lib.rs` 再 re-export | 全テスト green |
...
段階 0a を最初に切り出すのは、struct `Cache` の導入と trait rename をまとめると
diff が読みづらくなるため。意味論変更ゼロの rename を独立 commit にして bisectable
にする。
...
- `CacheImpl` trait の SIEVE-fit 改修 (現状 `&mut self get` 含めて SIEVE と整合済、
  別 issue で trait 設計再考の余地はあるが本スペックでは触らない)
...
| OQ2 | `CacheImpl` trait の SIEVE-fit 設計再考 (現状 placeholder のまま流用) | 低 (別 issue) |
...
| OQ6 | 並行版 (`SyncCache`?) を新 `Cache` の上に再構築 | 高 (本実装後の最優先) |
...
を実現する。`senba::Cache` として publishable な crate API の核を作る。
