...
| ライブラリ化 (`senba::Cache` 公開 API) | senba-sievecache-design, twitter-string-keys, senba-twitter-string-sweep |
...
で全 cap 完全一致 (0.4881 / 0.3216 / 0.1973)。**MQPS: cachesim ~2.5 (cap によらずほぼ一定)、
senba::sieve_orig 27.8〜42.9、senba::Cache 33〜35。ratio で 11〜17×**。ただし cachesim
側の timing には CSV パースが含まれるが senba 側は trace を Vec に展開後の replay loop
のみ計測。アルゴリズム比較ではなく **ハーネスを含むスタック比較**。SIEVE 単体比較は
FFI で C 実装を senba bench に取り込む follow-up (option b) が必要。`workload::file::libcachesim_csv_from_path`
