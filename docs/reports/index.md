...
2026-05-07 に j8 / `senba::Cache` 昇格 / c8 系列を反映して全面リライト。
過去 (A〜I 章 + M 章原文) はアーカイブ削除し、git log と `docs/reports/2026-05-0[3-5]-*.md`
参照に畳んだ。現在の関心は (A) library surface `senba::Cache` の API 拡張と内部最適化、
(B) 並行版 c8 系列の派生 (`c8a`/`c8e`/`c8s`) と sweep、(C) ST 路線の hot path 微細最適化、
(D) 計測軸 (skew / 大 V / churn / read-write 比) の拡張、(E) 兄弟比較 (S3-FIFO 等)。
棄却・実装済アイディアは末尾の表に圧縮。
...
senba::sieve_orig 27.5〜44.0 → 10〜17× gap**。follow-up で `traceConv` により
...
**昇格済み**: API 拡張のチェックリストを兼ねるため `docs/api-comparison.md` に
移動して living document 化した。以下は移動前の初版要約。

`senba::Cache` (`src/sieve_cache.rs`) の公開 API を **moka 0.12** (`moka::sync::Cache`) と
**lru** (`lru::LruCache`) を主軸、補助に **quick_cache** / **stretto** を加えてメソッド単位で
横並び比較したドキュメント。docs.rs の最新ページから 4 ライブラリの全公開メソッドを抽出し、
構築 / 基本 op / イテレーション / バルク / entry API / 立ち退き通知 / 並行モデル / 統計 /
派生 trait の 9 カテゴリで対照表を作成。**senba に事実として欠けている API を網羅列挙**:
builder / カスタム hasher / TTL / 重み付け / `peek` (非昇格 get) / `get_mut` /
`Borrow<Q>` 一般化 / `clear` / `iter` 系 / `drain` / `retain` / entry API /
`get_or_insert_with` 系 / eviction listener / hit-miss 統計 / `&self` 並行 / `Clone` /
`Debug` / async 版。`get` が `&mut self` なのは SIEVE の visited bit 更新由来で lru と同事情、
moka/quick_cache/stretto が `&self` で済むのは内部 lock + atomic で「ST/MT 構造の差が API
表面差にほぼ対応する」という観察を付記。本 crate の目的は SIEVE 比較研究であり API 網羅は
対象外、という前提のもと事実だけを記録した参照用ドキュメント。
