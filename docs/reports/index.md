...
| r 系列 (shard 間 routing × thread affinity の解空間) | r1-design, r1-results, r2-design, r2h-control-results |
| senba auto-shard heuristic | c17s-shard-heuristic |
...
### 2026-05-13-r1-vs-moka-sweep.md
仮説検証: c8 era (2026-05-06) の「c8 T=16 で moka 0.12 比 15.4×」が r1 まで保たれているかを直接実測 (これまで transitive 推論で繋いでいた lib viability 主張をデータで置換)。WSL2 で 400 cells × trials=3 を 9 分 22 秒、crashes 0、軸 = variants{c17s, r1@ways∈{1,8}, moka, mini_moka} × T∈{1..16} × workload{Zipf 4 cfg, Twitter Yang 3 cluster, ARC OLTP/DS1} × value{u64,string}, cap=4096/4000。**結果は予想を超過**: T=16 で senba (r1@8) は全 workload で moka を **7.2×–22.85×** 上回り、Zipf 1.4 read-heavy u64 で 207.69 Mops vs moka 9.09 Mops (VTune 予測 206 Mops を end-to-end で再現)。**p99 latency でも moka を 6×–39× 引き離す** (Zipf 1.4 RH u64 で moka 5075 ns vs r1@8 129 ns)。HR は **c17s / r1@1 が moka と全 workload で ±1pp 並走** (twitter_cluster019 では senba +7pp 取る)、policy 等価性が確認できた。c17s 単体でも moka 比 16× 出ており、優位は r1 routing affinity ではなく **c17s 段階の structural skeleton (SlotSize/AlignedTags/c-hoist/AVX2 + 64-shard fine-grained mutex) で既に支配的**。lib publish 前の必須は (1) bare metal 再計測 (WSL2 confound 対称想定の確認)、(2) ARC 8 trace 全 HR map (`external-lib-sweep.md` で W-TinyLFU が勝った P3/P6/S3 の補完)、(3) moka 0.13/0.14 追跡。データは `docs/benchmark/r1-vs-moka/`、48 figures + summary.md。

### 2026-05-13-r1-vs-moka-cap-sweep.md
仮説検証 (前報 §限定): cap-axis × ARC 全 11 preset × T=1/4/8/16 で `external-lib-sweep.md` の「scan-heavy ARC 大 cap で W-TinyLFU が SIEVE に +7–13pp HR 勝つ」構造が T=16 で再現するか。harness 拡張: `arc_preset_lookup` を `research/src/workload/arc_preset.rs` に lift (bench/bench_concurrent 共有)、bench_concurrent に `--arc-preset` 追加、**c17s/r1 の `[Shard; SHARDS]` を `Box<[Shard]>` 化** + SHARDS dispatch arms を 4..131072 に拡張 (`Cache::new(cap)` auto-shard と同等の `next_pow2(cap/64)` を harness 側で計算)。1140 cells × trials=3 を WSL2 で 1 時間 11 分、crashes 0。**主結論**: T=1 では external-lib-sweep の HR 反転構造が再現 (S3 cap=400k で moka +14.1pp, P3 cap=20k +8.7pp 等) する一方、**T=16 では逆転**: ARC DS1 cap=1M で c17s **+2.8pp**、P6 cap=160k で c17s **+4.5pp**、S3 cap=800k で c17s **+2.5pp**。1140 cell 全体で moka が c17s を HR +2pp 以上勝つのは twitter_cluster019 cap=1024 の 1 cell のみ。原因仮説は moka の write log dequeue が T=16 で 16× 圧力により saturate、admission accuracy が崩れる構造的問題 (per-op `run_pending_tasks` を呼ぶ external-lib-sweep の理想条件と非対称)。throughput は cap=1M–8M でも senba **3–25× moka**、**moka p99 が大 cap で 100–230 µs に張り付く** (DS1 cap=4M で **227 µs**) のに対し senba は <3 µs で SLA 主張に 2 桁差。**r1@8 は cap ≤ 16k 帯のみ有効**で cap=8M DS1 で c17s −51% / HR −57pp に劣後 — opt-in 境界として明確化。前報 §「HR drop が解けていない」評価のうち policy 層は **realistic concurrent (T≥2, no per-op sync) で実質解決**、残る課題は (i) single-thread embedded 用途、(ii) r1@ways≥2 shared-hot 帯のみ。データは `docs/benchmark/r1-vs-moka-cap-sweep/`、216 figures + summary.md。

### 2026-05-13-r2-design.md
r1 の弱点 (cluster006 HR drop 27pp / OLTP 21pp / DS1 cap=8M で HR −57pp) を 2 種の routing rule で攻める設計企画。重複蓄積を spillover で緩和する `r2s` (soft cap × α、α ∈ {1.25, 1.5, 2.0})、power-of-2 抽選の `r2p`、そして **r1 の thread affinity 寄与を切り分ける control `r2h`** (way = `mix(tls, hash) mod ways`) の 3 variant。仮説 H1 (r2h@8 cluster019 で r1 から大幅後退 → affinity essential) と H2 (r2h@8 ≈ c17s@shards=scaled×8 → ways は単なる sharding と等価) を別軸に立て、r2s/r2p 実装前に r2h で control 実験を打つ段階的判断構造。読み出し側は W-way probe が必須 (spillover lost insert 病の構造的回避)。

### 2026-05-13-r2h-control-results.md
**仮説 H1 部分肯定 + H2 強く否定**、r-series 全体に再考を迫る結果。224 cell × 3 trials = 672 rows を WSL2 で 5 分、crashes 0。**H1**: cluster019 cap=1024 T=16 で r2h は r1 から **−66.8%** 後退 (affinity essential)、しかし **cap ≥ 4096 では r2h ≈ r1** (差 ±3%) で affinity 寄与が消失。affinity が効くのは小 cap 帯のみ。**H2**: c17s_8x (= shard を 8 倍に増やすだけ) が **全 14 cell で r1/r2h を pareto dominate**: cluster019 cap=4096 で c17s_8x +447% / cluster006 cap=4096 で +173% / OLTP cap=512 で +608%、HR も +5〜+37pp 良い、p99 も 2〜5 倍低い。構造解釈は単純: r1@shards=64/ways=8 と c17s@shards=512/ways=1 は総 cap 同じだが mutex 数が 64 vs 512 で 8 倍差、これが Mops 差を支配する。affinity の writer-state 固定効果は二次的な副次効果に過ぎなかった。**含意**: (1) r1 design hypothesis が cap ≥ 4096 帯で否定、(2) **r2s/r2p 実装は中止** (c17s_8x が採用基準を既に満たしてしまった)、(3) senba auto-shard heuristic を `cap/64` から `cap/8` 系へ見直す sweep が lib publish への直接 path に。

### 2026-05-13-c17s-shard-heuristic.md
r2h control sweep の結論「shard を増やすだけで r1/r2h を pareto dominate」を広範な実 trace で検証 + sweet spot 特定。c17s × shards_mult ∈ {1,2,4,8,16} (= c1x..c16x) + r1@w=8 対照 × T={1,4,8,16} × 65 workload-cap cell = 4680 rows、WSL2 で 40 min、crashes 0。**56 (workload×T) cell の best 分布: c16x 36 (64%)、c8x 23、c4x 4、c2x 1、c1x 1、r1@8 2** (= 合成 Zipf 1.4 RH のみ)、現状 senba auto-shard (c1x = `cap/64`) が best の cell はゼロ。**Mops gain (c1x → c16x)**: cluster019 +514%、ARC OLTP **+584%**、Twitter cluster016/018 +183〜+199%、ARC DS1 +48%、Zipf 0.8 +71%。**HR cliff zone**: ARC P1/P3/P6/P8 (cap-fits 帯) で c16x が **−7〜−10pp の cliff** + Mops も regress、cap_per_shard=2 で SIEVE eviction が degenerate (local LRU 化)。**推奨 default = `cap/8` (= c8x)**: cliff の "1 段手前" で Mops gain +80〜+580%、HR drop ≤ −5pp の balanced sweet spot。c16x は Twitter/OLTP で更に +30% 取るが P-series リスクで default 化は危険。**結論**: senba auto-shard を `next_pow2(cap/64)` → `next_pow2(cap/8)` に変更推奨、`MIN_PER_SHARD ≈ 4` floor を要設置、r-series は確定的に artifact 凍結。next: lib PR + perf-gate + moka 比較を c8x で取り直し。

### 2026-05-13-senba-concurrent-cache-design.md
c17s research variant を `senba::concurrent::Cache` として publishable surface に格上げした実装ログ。c17s reader の `ptr::read(entry) → V::clone` で発生する old V drop race を `Entry.value: Arc<V>` + `crossbeam-epoch::Guard::defer_unchecked` で塞ぎ、reader の `epoch::pin` で writer の deferred drop を hold する moka 互換 soundness model に到達。**`V: Clone + Send + Sync + 'static`** (V: Copy 制約を撤廃)、`&self` API、`get/remove -> Option<V>`、`Cache::new` は `next_pow2(cap/8)` auto-shard + `MIN_PER_SHARD=4` floor。`senba::concurrent::PartitionedCache` を同 release で削除 (c17s_8x が pareto-dominate)、`bench_concurrent` / `bench_vtune_concurrent` の `--variant partitioned` を `--variant senba_concurrent` に置換。テスト 120 本通過 (V = String chaos 含む)。perf-gate 再走と moka 比較は follow-up。

### 2026-05-13-senba-concurrent-vs-c17s.md
仮説検証: `senba::concurrent::Cache` (lib) は c17s skeleton から `Entry.value: Arc<V>` + `epoch::pin/defer_unchecked` + `WriterState::{free_ids, next_fresh_id}` + `Mutex<Box<WriterState>>` を追加しているが、これらが reader / writer hot path にどれだけ乗っているかを未計測のまま publish していた。`bench_concurrent` で c17s vs senba_concurrent を `--shards 512` (= cap/8 c8x sweet spot) 固定、cap=4096、Zipf {0.8/1.0/1.4} × T {1/4/8/16} × {gim, read-heavy} × 3 trials = 144 cells 計測。**結果: 全 cell 退行、median −34%、worst −63.4%** (gim T=16 z=1.4 で c17s 184.25 → senba 67.35 Mops)。スケール構造: T=1 median −16% → T=16 −48% (Arc strong-count cache-line ping-pong の signature)、skew 0.8 −22% → 1.4 −44% (hot-key 共有 ArcInner 集中)、read-heavy median −22% < gim −44% (gim は writer path の `Arc::new` per miss + `epoch::pin` の二重コスト)。`research/benches/sieve_cache_perf.rs` は single-thread `u64` only で contention 軸が出ないため、T=1 帯の 15-25% も noise floor 内に隠れ、構造退行が一切検出できていなかった。Follow-up 案: (1) `V: Copy` specialization で Arc/epoch を Copy path から完全に剥がす、(2) `RwLock<ShardInner>` baseline (`sieve_rw0`) を計測 — c-series 全期間で未測定、(3) concurrent perf-gate を本 sweep の regression hotspot から 1 cell ずつ抜き出して baseline 化。データは `docs/benchmark/senba-concurrent-vs-c17s/`、48 cells 表 + Mops 棒グラフ。

### 2026-05-14-r3-vs-c17s.md
仮説検証 (前報 follow-up ②): `senba::concurrent::Cache` の Arc<V>+epoch を `parking_lot::RwLock<ShardInner>` 1 本に置き換える `sieve_r3` を組み、reader hot path の atomic を c17s (~8-10) より少ない ~5 (RwLock acquire+release + visited fetch_or) にすれば c17s に勝ち senba_concurrent を +20% 以上で上回るのが採用基準だった。72 cells × 3 trials = 216 rows、crashes 0。**仮説は両側棄却**: r3/c17s median **−17.1%** / worst **−76.7%** (T=16 z=1.4 read-heavy で c17s 137.9 → r3 32.2 Mops)、r3/senba_concurrent は median **+12.2%** / best **+75.9%** (gim z=0.8 T=8) で lib より速いが、c17s には届かない。スケール構造: T=1 median −2.2% → T=16 −43.2%、z=0.8 −11.1% → z=1.4 −46.1%、gim −12.4% < read-heavy −19.8%。**Mechanism: parking_lot::RwLock の reader counter が single shared atomic** で、hot shard に 16 thread が同時 read-lock を取ると counter atomic が MESI ping-pong する。**c17s lock-free reader は共有 atomic を一切書かない** ので MESI Shared のまま 16 core 並列読みできることが支配的優位源だった。r3 が勝つのは低 skew + low T (T=1 z=0.8 gim +8%)。実装簡素化: senba::concurrent から inherit していた `free_ids` + `next_fresh_id` を `remove()` 時 swap-with-last compact で代替し、ShardInner を 26B 縮小 (perf neutral 確認済み)。**Implication: lib 置換先として r3 reject**、残された主 path は follow-up ① の `V: Copy` specialization (c17s lock-free reader を保ったまま Arc を剥がす)。r3 は research artifact として保持、parking_lot::RwLock の reader counter limit を示す引き合いに残す価値あり。データ: `docs/benchmark/r3-vs-c17s/`。

### 2026-05-14-arc-less-concurrent-design.md
設計 spec (実装前): senba::concurrent の Arc<V> 退行 (−34%/−63%) を解くため、c17s の **entry-version seqlock を snapshot consistency (race α) に**、crossbeam-epoch を **V の heap reclamation (race β) に** 責務分離して合成する `c19s` 仮称の design。Arc は撤去、Entry に V 直置き、reader は `epoch::pin` + 既存 seqlock + `V::clone` 直呼び。reader hot path の shared atomic write を c17s 同様に **0 本** (visited 既 set 時) に戻す。`mem::needs_drop::<V>()` の const branch で V: Copy を monomorphize-time に Copy 特殊化フュージョン (c17s 同等の asm 生成期待)。**race β の正当性を crossbeam-epoch の global epoch synchronization (case I/II/III) で形式的に証明**、R8 に compiler_fence(Acquire) を明示追加 (c17s 暗黙依存を顕在化)。期待性能: V=u64 で c17s 同等、V=String hot-key T=16 で現 lib から +30〜+50%。実装の難所として (i) ManuallyDrop wrap への Entry 形状変更と Shard::drop 責務、(ii) loom 非対応の crossbeam-epoch に対する verification 代替戦略 (miri + TSan stress)、(iii) reclamation backlog の bound 実測、(iv) `mem::needs_drop` const-fold が opt-level=3 で確実に効くかの cargo asm 検証、を列挙。Phase 1-4 (research variant `sieve_c19s` → miri/TSan → 3-way perf sweep → lib integration) で段階的に検証する。

### 2026-05-16-senba-concurrent-r4-migration.md
仮説検証 (前報 follow-up): `2026-05-15-r4-vs-c17s.md` の 432-cell Zipf sweep で確認した「r4 engine が Arc<V> 系 senba::concurrent を Pareto improve、c17s と V=u64 で ±0.3% 並走」結果は Zipf 単体でなく実 trace でも再現するはず、を v0.4.0 swap commit と同時に検証する。あわせて moka / mini-moka adapter を `Cache::new` (default `RandomState` + 遅延 grow) から **`builder().initial_capacity(cap).build_with_hasher(senba::Xxh3Build)`** に切り替えて hasher / 容量を senba と揃え、その上で再計測。**Full sweep 結果** (2048 cells × 3 trials = 6144 row、crashes 0、Zipf + libcachesim + Twitter-Yang 5 cluster + ARC 22 preset): V=u64 senba_concurrent vs c17s で Zipf median **−0.3%** / Twitter **−2.8%** / ARC **+1.0%**、median は parity (worst FAIL は cap_per_shard 極小の P-series)。V=String は soundness 確保コストで Zipf median **−11.5%**、Twitter **−31.3%**、ARC **−34.3%** — `2026-05-14-arc-less-concurrent-design.md` §9.3 の予測 (-20〜-30%) と整合、Zipf 高 T cell では senba_concurrent が +60〜+100% で逆転。**Null result**: moka / mini_moka adapter を fair tuning (Xxh3Build + initial_capacity) で組み直しても ARC u64 median **−95.0%** / Zipf u64 median **−90.0%**、前回 sweep からの median 差は ≤ 1pp。moka 系の支配コストは hash / rehash ではなく W-TinyLFU bookkeeping (read/write log + sketch + admission) そのもの、と確認。ASan clean (race β/γ 構造的閉鎖)、TSan 11 seqlock false-positives (旧 16 から Arc 関連 5 件消失)、`sieve_concurrent_perf` baseline 保存。data: `docs/benchmark/concurrent-full-sweep/`。

### 2026-05-15-r4-vs-c17s.md
仮説検証 (前報 follow-up ①、設計 `2026-05-14-arc-less-concurrent-design.md`): `c19s` を `sieve_r4` として実装し、c17s + crossbeam-epoch (refcount 抜き defer) で Arc<V> 退行を構造的に解消できるかを 432-cell sweep で計測 (3 variant × 4 T × 3 skew × 2 mix × 2 value × 3 trial、crashes 0)。**V=u64 accept PASS** (r4 vs c17s median **+0.3%** / worst **−8.4%** で `mem::needs_drop` の monomorphize-time fold が完全機能、`cargo asm` で V=u64 の `crossbeam_epoch::pin` が 0 件、V=String では 2 件の binary symbol 確認)。**V=String accept FAIL on target +30%** (r4 vs senba median **+21.1%** / worst **+10.2%**) — Pareto improvement は確認、ただし target 未達。要因: senba の Arc strong-count cross-core ping-pong は high-skew / high-T で最も顕在化する (skew=1.4 T=16 で +50%) が、low-skew では Arc bottleneck が薄く、r4 の構造的優位 (atomic write 0) も縮む (skew=0.8 T=1 で +14.9%)。Phase 2 sanitizer: ASan clean (race β/γ 実機 confirm)、TSan は 16 false-positive (seqlock pattern、kernel seqlock + KCSAN と同じ、`docs/benchmark/r4-sanitizer/findings.md`)、Miri skip (AVX2 cfg 非互換)。API 差分: r4 の `insert` は Path C 経由で常に None (旧 K, V を defer closure に move、lib API contract `Option<(K,V)>` 維持には callback 形が必要)。**Implication**: V=u64 主用途は Phase 4 進行可、V=String は target 緩和 (+20%) または high-skew 限定で進行可、low-skew V=String を強く保証するなら追加最適化 (per-shard collector、hazard pointer 等) が必要。data: `docs/benchmark/r4-vs-c17s/`、figures + cell 別 PASS/FAIL は `figures/regression_summary.md`。

### 2026-05-15-concurrent-scalar-fallback.md
仮説検証 (commit `94e3269` の perf-gate): `senba::concurrent::Cache` の reader 走査を AVX2+BMI1 と portable scalar の runtime dispatch (`has_avx2_bmi1: bool` field + 内側 hot path への bool 引数 + `#[inline]` dispatcher) に分けた変更が CLAUDE.md gate (>5% regression on any cell) を踏むかを HEAD^ baseline 比較で確認。**4 cell 全 PASS、worst +2.73%** (`zipf14_u64_t16`, p=0.00 だが criterion 自身の noise_threshold=0.02 内)、その他 cell は +0.57〜+1.29%。退行は u64 hot path に集中し string cell では `V::clone` + epoch defer の per-op コストに希釈されて表面化しない (dispatcher branch が +0.1 ns/op オーダーで u64 = 8 ns/op に直接乗る構造)。AVX2 intrinsic body は不変、SIMD 走査自体は等価で走っている (cells の絶対値が 1-3% で済む傍証)。Follow-ups (`SENBA_FORCE_SCALAR=1` での scalar twin 単独 characterisation、env lookup の `OnceLock` cache 化、aarch64 cross-check の CI 追加) は perf gate とは別扱いで polish 案件。

### 2026-05-15-find-evict-pos-optimization-design.md
設計書 (実装前): `find_evict_pos` 周辺の instruction-level 最適化を Phase 0..6 に分解。P1 (live_mask の `>>` 化で分岐除去) / P2 (rotate+tzcnt 一発化で 2 分岐 + 1 tzcnt 削減) / P3 (`shift_visited_down_in_place` の u128 → u64) / P4 (BMI2 `bzhi` で walked-mask 1 op) / P5 (PDEP/PEXT) / P6 (clear と shift fuse) の段階構造、依存表、各 Phase の独立 commit + perf-gate AB + `>5% 退行で revert` exit 条件。リスクとして u128 mask の重さと rorx 出力可否を §4.6 で列挙、register pressure は事前評価から漏れていた (結果報告 `2026-05-16-find-evict-pos-optimization-results.md` 参照)。

### 2026-05-16-find-evict-pos-optimization-results.md
仮説検証 (前報 follow-up): 設計書 §4-7 の Cut A (P1+P2+P4+Step A distributivity) と Cut B (P3 `shift_visited_down_in_place` u128 → u64) を実装、perf-gate + Twitter cross-check (4 cluster × 4 cap × 5 rep × 2 dataset) で検証。**Cut A 全 revert** — `find_evict_pos` を完全 branchless な rotate+tzcnt 一発形に潰す試みは eviction-dominant 低スキューで Twitter cluster034 cap=16384 −8.7% 改善を出す一方、V=String / K=String workload で perf-gate +5.7% / Twitter +5.73% が安定再現、reg pressure を救うはずだった Step A distributivity も asm spill 数を動かさず +5% 退行解消できず。原因は旧コードの common path branch が予測ヒットで実コスト ~0、削除しても得るものなし + 新コード +3 uops が直接 eviction cost に乗る。P4 BMI2 dispatch は `#[target_feature]` inline barrier で out-of-line call 化し +9.3% 退行 — `#[target_feature]` 関数を非 target_feature な caller から呼ぶ盲点。**驚き**: branchless で構造的に最も美しい形が perf で勝てない、common case で 2 分岐削除はそもそも free だった。**Cut B 採択** — perf-gate `insert_u64_string` で 2 run 安定 +5% (LLVM が `(a&m)|(b&~m) → b^((a^b)&m)` カノニカル化で XOR トリックを選び dep chain 6-deep に延長) を踏むが、Twitter K=u64 で 13/16 セル改善 (最大 cluster034 cap=16384 **−14.20%**)、K=String で 7/16 改善 / 5/16 mild 退行 (全て +3.62% 以下、>5% ゼロ) で memory `feedback_perf_gate_diversity` の "criterion 単独判断は危険" 適用、実 trace 32 セルの方向一致を主 signal として採用。MSRV 1.85 で `unchecked_shl` が使えず XOR トリック lowering を押し戻せなかったのは制約として記録。data: `/tmp/twitter-{baseline,u64-cutB,string-{baseline,cutB}}.csv` (永続化 follow-up)。**§8 追記 (post-mortem)**: §5.3 で受容した XOR トリック lowering を `cfg(target_feature="bmi2")` の明示 `_bzhi_u64` dual-path で押し戻せないか検証 → **逆に大幅退行**。BMI2 target で 7/8 セル退行 / insert_u64 +15.89%、原因は LLVM が `visited & ((1<<pos)-1)` を `bzhi(visited, pos)` 単一命令に **fuse** していたところ、明示 intrinsic で mask を先に materialize すると fusion が壊れ critical path +1c。baseline target では code 機械命令 bit-identical (asm diff で確定)、perf-gate signal の正体は cfg 属性が debug info サイズを変えた ELF レイアウト shift で完全 layout artifact。dual-path 不採用、Cut B u64 idiom が両 target で最適。同 commit で perf-gate config を `200/8s/2s` に強化 (run 間 ±6-8pp の振れを解消、CI ±0.5pp 程度)。
