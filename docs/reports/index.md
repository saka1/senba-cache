...
仮説検証 (前報 follow-up): `2026-05-15-r4-vs-c17s.md` の 432-cell Zipf sweep で確認した「r4 engine が Arc<V> 系 senba::concurrent を Pareto improve、c17s と V=u64 で ±0.3% 並走」結果は Zipf 単体でなく実 trace でも再現するはず、を v0.4.0 swap commit と同時に検証する。`src/concurrent/cache/shard.rs` を Arc<V> + epoch refcount → r4 engine (ManuallyDrop<K/V> + epoch defer of moved-out K/V) に置換、`insert` を `()` 返しに API 破壊、`insert_with(K, V, on_evict)` callback を追加 (race β + race γ 同時 closure)、`V/K: Sync` を bound から外す。senba を v0.3.0 → v0.4.0。`docs/benchmark/concurrent-full-sweep/` を 4 phase 構成 (Zipf / NSDI24 libCacheSim / Twitter-Yang 5 cluster / ARC 22 preset) で新設、4-way (新 senba_concurrent / 旧 c17s / moka / mini_moka) sweep。**Full sweep 結果** (2048 cells × 3 trials = 6144 row、crashes 0、Zipf + libcachesim + Twitter-Yang 5 cluster + ARC 22 preset): V=u64 senba_concurrent vs c17s で Zipf median **−1.3%** / Twitter **−3.1%** / ARC **+0.9%**、軸全体で parity (≥80% cell が ±5% 内、worst は cap_per_shard 極小の P-series で出る)。V=String は soundness 確保コストが trace 系列で違い、Zipf median **−10.0%**、Twitter **−30.3%**、ARC **−33.9%** — c17s は race β unsound で raw V clone で速いだけ、設計 `2026-05-14-arc-less-concurrent-design.md` §9.3 の予測 (-20〜-30%) と整合。Zipf 高 T cell では senba_concurrent が +100% 以上で逆転、moka/mini_moka は ARC u64 で median −95% / worst −99%。**結論**: 旧 senba::concurrent (Arc) からの median −34%/worst −63% 退行を r4 lift で全 cell 救済、v0.4.0 を release candidate とする根拠として fix。 WSL2 計測 bias caveat あり、release タグ前に V=String × skew=1.4 × T=16 を bare Linux / Windows native で 1 セル cross-check 必要。ASan clean (race β/γ 構造的閉鎖を実機 confirm)、TSan 11 seqlock false-positives (旧 16 から Arc 関連 5 件消失)、新規 `sieve_concurrent_perf` baseline 保存。data: `docs/benchmark/concurrent-full-sweep/`。
