...
| `peek_mut` (非昇格 mut) | 〇 | x | O | x | x |
| `peek_lru` / `peek_mru` | x | x | O | x | x |
| `get_mut` | 〇 | x | O | x | O |
| `get_key_value` / `peek_key_value` | 〇 | x | O (`get_key_value` 系) | x | x |
...
| 統計 (hits/misses) | 〇 (`stats()`) | x | x | O (feature) | O (`metrics`) |
...
pub fn peek<Q>(&self, key: &Q) -> Option<&V>                                   // 〇 added
    where K: Borrow<Q>, Q: Hash + Eq + ?Sized;
pub fn peek_mut<Q>(&mut self, key: &Q) -> Option<&mut V>                       // 〇 added
    where K: Borrow<Q>, Q: Hash + Eq + ?Sized;
pub fn get_key_value<Q>(&mut self, key: &Q) -> Option<(&K, &V)>                // 〇 added
    where K: Borrow<Q>, Q: Hash + Eq + ?Sized;
pub fn peek_key_value<Q>(&self, key: &Q) -> Option<(&K, &V)>                   // 〇 added
    where K: Borrow<Q>, Q: Hash + Eq + ?Sized;
...
- `peek_mut` は `peek` の `&mut V` 版で、こちらも VISITED を立てない(〇 追加済み)。
  「カウンタを増やす」「TTL タイムスタンプを更新する」など論理的にアクセスでない
  in-place 更新で使う。`get_mut` は VISITED を立てるのでこの目的には使えない。
- `get_key_value` / `peek_key_value` は `(&K, &V)` を返す系(〇 追加済み)。
  前者は `get` 同様 VISITED を立て、後者は `peek` 同様非昇格。`Borrow<Q>`
  経由で引いた際に、格納されている canonical な `&K` を取り出せる。
...
`keys` / `values` / `IntoIterator for &Cache` / `&mut Cache` / `drain()` を
追加して、参照系・更新系・consume 系を一通り揃えた。値所有
`IntoIterator for Cache` は `drain().collect()` の sugar 以上の意味を持たない
(SIEVE は保存に値する traversal 順を持たないため、所有権を取って yield
する動機が `Vec::into_iter` 等とは異なる)ので、`drain` のみ提供して owned
`IntoIterator` は実装しない方針とした。
...
- `Cache::stats() -> Stats { hits, misses, insertions, evictions }` を
  公開(〇 追加済み)。プラン:
  - `hits` / `misses`: 昇格 lookup(`get` / `get_mut` / `get_key_value` /
    `get_or_insert_with` の lookup 部分)のみカウント。`peek*` /
    `contains_key` は非昇格プローブとしてカウントしない。
  - `insertions`: `insert` 呼び出し全部(replace 含む)+ `get_or_insert_with`
    の miss path。
  - `evictions`: 容量起因 evict のみ(insert 内)。`remove` / `clear` /
    `retain` の drop はカウントしない(明示削除なので別経路)。
- 実装はカウンタを per-shard に置いた plain `u64`(`AtomicU64` 不採用)。
  ST 構造で `&mut self` 既必須なので非 atomic で十分。`stats()` 呼び出し
  時にだけ全 shard を線形に合算する。
- perf-gate(`insert_u64` / `mixed_u64` / `insert_string`)は導入前後で
  全シナリオ noise 範囲内(±2%)。OoO が hit/miss/insert の add を
  hot path の依存チェーンと並列に retire するため実質ゼロコスト。
...
`hits` / `misses` / `insertions` / `evictions` の 4 カウンタは出るように
なった。残るのは:

- 原因別 eviction(`Replaced` / `Size` / `Explicit` / [TTL があれば `Expired`])。
  Caffeine `CacheStats` / moka `RemovalCause` / stretto `Metrics` がここを
  細分化している。senba は容量 evict のみカウントしているので、明示削除や
  replace の観測は別経路(`remove` 戻り値、`insert` 戻り値、listener)が必要。
- eviction listener(全 evict を hook する callback)。これが入ると `insert`
  hot path に無条件 branch が乗るため、本 crate は thin 路線として導入しない
  方針(rich 系は将来別 facade で別途検討)。
...
- `get_key_value` / `peek_key_value` 〇
...
- `IntoIterator` 〇 (`&Cache` / `&mut Cache`。値所有版は `drain` と冗長なので実装しない方針)
...
- hit / miss / eviction カウンタ 〇 (`Cache::stats() -> Stats`、4 フィールド: hits/misses/insertions/evictions)
- 原因別 eviction(replaced / size / explicit を区別する `RemovalCause` 相当)
...
- `Clone` 〇 / `Debug` 〇(`Default` は容量が deliberate なパラメータで意味のある
  デフォルト値が無いため意図的に見送り — `lru` / `quick_cache` / `moka` も同じ理由で未実装)
- 名前(`name()`)
- Serde サポート
...
- **2026-05-07**: `IntoIterator` を `&Cache` / `&mut Cache` に実装。`for (k, v) in &cache`
  ...
  サマリ表 / §9 を 〇 に更新。
- **2026-05-07**: §9 の残ギャップを再整理。`Default` は容量が deliberate な
  パラメータでデフォルト値を選びようがないため見送り(`lru` / `quick_cache` /
  `moka` も未実装で同じ理由)。値所有 `IntoIterator for Cache` は SIEVE に
  保存すべき traversal 順がなく `drain().collect()` の sugar 以上にならないため
  実装しない方針に変更。consume 系のギャップは `drain()` 単体で埋める。
  §3 / §9 を更新。
- **2026-05-07**: `Cache::drain` を実装。`std::vec::Drain` と同じ意味論
  (Drain 構築時点で cache は論理的に空、Drain Drop で残りのエントリを drop、
  `mem::forget` でリーク発生だがキャッシュは consistent)。`Drain::new` は
  per-shard で `tags[..old_len]` を EMPTY にゼロクリアして `len/hand=0` にし、
  invariant I8(live ids = `0..len`)を根拠に、エントリ実体は触らず id を
  0 から走査する Iterator として実装。タグの事前ゼロは SIMD `find` の
  scan window が round-up で len を越えて読む点に対する soundness 担保
  (`mem::forget(drain)` 後の insert がスタール LIVE タグに当たらないようにするため)。
  drain は `clear` / `retain` と同じく `evictions` をカウントしない。
  ExactSizeIterator / FusedIterator も実装。perf-gate (`insert_u64` / `mixed_u64`
  / `insert_string`) は noise 範囲内で hot path 影響なし。サマリ表 / §3 / §4 /
  §9 を 〇 に更新。
...
- **2026-05-07**: `Cache::peek_mut` / `Cache::get_key_value` / `Cache::peek_key_value`
  を追加。`peek_mut` は `&mut V` を返す非昇格 lookup(VISITED を立てない)、
  `get_key_value` / `peek_key_value` は `(&K, &V)` を返す昇格 / 非昇格ペア。
  いずれも `K: Borrow<Q>` 一般化に対応。実装は per-shard の既存 `find` の戻り値
  から id を引いて `Entry { key, value }` の該当フィールドを返すだけで、
  hot path の構造には触っていない。サマリ表 / §2 / §9 を 〇 に更新。
- **2026-05-07**: `Extend<(K, V)>` および `Extend<(&K, &V)>` (where `K, V: Copy`) を
  ...
  サマリ表 / §4 / §9 を 〇 に更新。
- **2026-05-07**: `Cache::stats() -> Stats { hits, misses, insertions, evictions: u64 }`
  を実装。per-shard に plain `u64` カウンタ 4 本を持って、`stats()` 呼び出し時に
  合算する形(`AtomicU64` 不採用 — ST `&mut self` 構造のため)。カウント対象は
  昇格 lookup(`get` / `get_mut` / `get_key_value` / `get_or_insert_with` の
  lookup 部分)・全 `insert` 呼び出し・容量起因 evict のみで、`peek*` / `contains_key`
  / `remove` / `clear` / `retain` はカウントしない。perf-gate
  (`insert_u64` / `mixed_u64` / `insert_string`)は全シナリオ noise 範囲内(±2%)で、
  事前見込みどおり OoO が hit/miss/insert の add を hot path から切り離して
  実質ゼロコストになっている。サマリ表 / §7 / §9 を 〇 に更新。
