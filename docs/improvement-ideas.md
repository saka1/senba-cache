...
| `sieve_j8` (M5.4) **設計のみ** | tag に entry_id 埋込 + entries arena cap-only + free_list 廃止。**inline 20 B/cap で orig (25) を下回る**狙い、throughput は per_shard ≤ 64 で j7 比 +0.5〜1 ns 退行見込み (noise 圏) | `j8-design` |
...
| **M5.4 (= `sieve_j8` 設計)** | M5.3 + tag bit に entry_id 埋込 (別 order 配列を消す) + free_list 廃止 (`entry_id = len` warm-up / evict 戻り値 pass-through steady) | inline **20 B/cap** で orig (25) より軽い。詳細設計は `docs/reports/2026-05-05-j8-design.md`。`per_shard ≤ 64` 構造的上限、false-match 1/256 (j7 1/16384 比劣化、ただし sweet spot で throughput 退行 ≤ 1 ns 見込み) |
...
3. **M5.4 (j8、設計済 / 実装着手前)** — M5.3 + tag 内 entry_id embed + free_list 廃止。**inline 20 B/cap で orig 25 を下回る**狙い。設計書 `docs/reports/2026-05-05-j8-design.md`。机上検討では sweet spot per_shard ≤ 64 で退行 ≤ 1 ns。memfair で「j 系列が orig を absolute に抜く」初の設計候補。
4. **memory-fair sweep on j7** — j7 の inline B/cap (実効 18) で揃えた cap で orig (25) / j5 (25) と head-to-head。
5. **M1.1 or M1.2 (slack 削減)** — j7 と直交。small-cap 帯ほぼ無痛。倍率 vs ns/op vs memory の Pareto sweep。M5.4 (j8) と組合せると更に物理 17 B/cap まで詰まる (j8 設計書 OQ2)。
...
- ~~M5.4 設計~~ → 完了 (`j8-design`)
- **次**: M5.4 (j8) 実装 + Twitter sweep + memfair sweep
- 並行: j7 の memory-fair sweep + cluster019 (scan-heavy) 追加検証
- M1 系の Pareto sweep を j7 / j8 ベースで
- 必要に応じて M3 を重ねる
