#!/usr/bin/env python3
"""cap=1024 sweep の summary を生成する。

入力: data/results.csv (run.sh 出力、append-only)
出力:
  - figures/fig_pareto_overlay_all.png : 全 workload 1 図 overlay (HR drop vs Mops gain)
  - figures/summary.md                  : 採用領域カウント + key cell 表 + variant best cell

per-workload / heatmap / scalability の個別図は廃止 (測定データから随時導出可能、
overlay 1 枚 + 表で全体傾向を見るのが実用的だった、という 2026-05-12 sweep の学び)。
...
def write_summary(pairs: pd.DataFrame, agg: pd.DataFrame, outdir: Path):
    ...
    key_cells.append(("arc_OLTP_cap1000", "u64", 16, "partitioned", 16, "HR-sensitive"))
...
def main():
    ...
    print(f"summary written: {out_dir}/summary.md, {out_dir}/fig_pareto_overlay_all.png", file=sys.stderr)
